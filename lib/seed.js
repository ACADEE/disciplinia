...
const CONFIG_DEFAUT = {
  entreprise: {
    ...
    ville: "Annecy",
    signataire: "Anne-Sophie Bertrand",
...
function seed({ reset = false } = {}) {
  if (reset || !store.getConfig()) store.saveConfig(CONFIG_DEFAUT);
  if (reset || store.getSalaries().length === 0) store.saveSalaries(SALARIES);
  if (reset || store.getDossiers().length === 0) store.saveDossiers(seedDossiers());
}

if (require.main === module) {
  seed({ reset: process.argv.includes("--reset") });
  console.log("Données de démonstration (ré)initialisées.");
}
