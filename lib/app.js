...
app.get("/api/dashboard", ah(async (req, res) => {
  ...
  for (const d of enCours) {
    ...
    if (jours > 60) alertes.push({ id: d.id, reference: d.reference, salarieNom: d.salarieNom, texte: `Prescription probable (${jours} j depuis connaissance du fait)`, niveau: "danger" });
    else if (jours > 45) alertes.push({ id: d.id, reference: d.reference, salarieNom: d.salarieNom, texte: `Prescription dans ${60 - jours} j`, niveau: "warning" });
  }
